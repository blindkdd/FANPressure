...
class CornicheEnv(gym.Env):
    ...
    def _subscribeALL(self):
        for edge in self.allConnectedEdges:
            self.conn.edge.subscribe(edge, (tc.LAST_STEP_VEHICLE_NUMBER,tc.LAST_STEP_OCCUPANCY))

        for edge in self.allEdges:
            self.conn.edge.subscribe(edge, [tc.VAR_WAITING_TIME,0x14,0x60])
    ...
    def _observeState(self):
        reward = []
        observations = []
        waiting_time = sum([self.conn.edge.getSubscriptionResults(edge)[tc.VAR_WAITING_TIME] for edge in self.allEdges])
        halting_cars = sum([self.conn.edge.getSubscriptionResults(edge)[0x14] for edge in self.allEdges])
        co2 = sum([self.conn.edge.getSubscriptionResults(edge)[0x60] for edge in self.allEdges])
        measures = {"completed_trips":self.completed_trips,"edgesIndex":self.allConnectedEdges,"waitingTime":waiting_time,"queue":halting_cars,"co2":co2}
        ...
        return (np.array(observations),self._observeStateOLD()[0]),reward,measures
