...
g.ndata['entered'] = torch.zeros((g.number_of_nodes(),1)).cpu()
...
class PredictEdgeProbModule(nn.Module):
    ...
    def forward(self, edge):
        ...
        selfloop = (edge.src['features'] == edge.dst['features']).type(torch.FloatTensor).cpu().unsqueeze(1)

        enabled = (selfloop != edge.src['free']).type(torch.FloatTensor).cpu()
...
def flow_message_func(edges):
    ...
    selfloop = (edges.src['features'] == edges.dst['features']).type(torch.FloatTensor).cpu().unsqueeze(1)
